...
export default class PlayerRepository implements InterfaceRepository {
  ...
  public async getCollection(
    ...
    promises.push(this.mysqlClient.execute(
      `SELECT HEX(player.uuid) AS uuid, HEX(player.server) AS server, player.created, player.updated, SUM(CASE WHEN entry.giveable = 1 THEN entry.sips ELSE 0 END) AS sips_giveable, SUM(CASE WHEN entry.giveable = 1 THEN entry.shots ELSE 0 END) AS shots_giveable, SUM(CASE WHEN entry.giveable = 0 THEN entry.sips ELSE 0 END) AS sips_remaining, SUM(CASE WHEN entry.giveable = 0 THEN entry.shots ELSE 0 END) AS shots_remaining, -SUM(CASE WHEN entry.sips < 0 AND entry.giveable = 0 THEN entry.sips ELSE 0 END) as sips_taken, -SUM(CASE WHEN entry.shots < 0 AND entry.giveable = 0 THEN entry.shots ELSE 0 END) as shots_taken FROM player LEFT JOIN entry ON player.uuid = entry.player WHERE player.server = UNHEX(REPLACE(?, '-', '')) AND entry.server = UNHEX(REPLACE(?, '-', '')) GROUP BY player.uuid, player.server ORDER BY player.created DESC LIMIT ? OFFSET ?`,
      [server, server, limit, offset],
    ));

    promises.push(this.mysqlClient.execute(
      `SELECT COUNT(player.uuid) AS total FROM player LEFT JOIN entry ON player.uuid = entry.player WHERE player.server = UNHEX(REPLACE(?, '-', '')) AND entry.server = UNHEX(REPLACE(?, '-', ''))`,
      [server, server],
    ));
